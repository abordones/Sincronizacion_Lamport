"""
Cliente del sistema distribuido que se conecta a un servidor UNAP remoto en la nube.
"""
...
# Variables de entorno - CONFIGURAR PARA SERVIDOR REMOTO
PROCESS_ID = int(os.getenv('PROCESS_ID', 1))
PROCESS_NAME = os.getenv('PROCESS_NAME', 'Cliente-Local')
# URL del servidor remoto - CAMBIAR ESTA URL
SERVER_URL = os.getenv('SERVER_URL', 'http://tu-servidor.com')  # Cambiar por tu dominio
...
<head>
    ...
    <style>
        ...
        .server-info { background: #e2e3e5; padding: 10px; border-radius: 5px; margin: 10px 0; }
    </style>
    <script>
...
<body>
    <div class="container">
        ...
        <div class="connection-status {{ 'connected' if server_connected else 'disconnected' }}">
            <strong>Estado del Servidor Remoto:</strong> {{ 'Conectado' if server_connected else 'Desconectado' }}
        </div>
        
        <div class="server-info">
            <h3>Información del Servidor:</h3>
            <p><strong>URL del Servidor:</strong> {{ server_url }}</p>
            <p><strong>ID del Proceso Local:</strong> {{ process_id }}</p>
            <p><strong>Nombre del Proceso:</strong> {{ process_name }}</p>
        </div>
...
def send_message_to_server(message: str):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            # Actualizar reloj con la respuesta del servidor
            lamport_clock.receive_event(data.get('server_clock', 0))
            add_event(f"Mensaje enviado a servidor remoto: '{message}' - Reloj: {lamport_clock.get_time()}")
            return True
        else:
...
def index():
    ...
    # Verificar conexión con el servidor remoto
...
if __name__ == '__main__':
    ...
    # Intentar registrar con el servidor remoto
    add_event(f"Cliente {PROCESS_NAME} iniciado - Reloj inicial: {lamport_clock.get_time()}")
    add_event(f"Conectando a servidor remoto: {SERVER_URL}")
    ...
    if register_with_server():
        print(f"✅ Cliente {PROCESS_NAME} registrado exitosamente con servidor remoto")
    else:
        print(f"⚠️ Cliente {PROCESS_NAME} no pudo registrarse inicialmente")
        print(f"💡 Verifica que el servidor remoto esté disponible en: {SERVER_URL}")
